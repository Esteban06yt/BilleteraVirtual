package co.edu.uniquindio.poo.billeteravirtual.viewController;

import javafx.event.ActionEvent;
import javafx.scene.control.Button;
import javafx.scene.control.TableColumn;
import javafx.scene.control.TableView;
import javafx.scene.control.TextField;
import javafx.scene.image.ImageView;
import javafx.scene.layout.AnchorPane;
import javafx.scene.text.Text;

public class CrudUsuarioViewController {

    public AnchorPane ap_crudUsuario;
    public Text txt_titulo;
    public Text txt_subtitulo;
    public ImageView img_logo;
    public TableView tb_listaUsuarios;
    public TableColumn tbc_nombre;
    public TableColumn tbc_cedula;
    public TableColumn tbc_telefono;
    public TableColumn tbc_correo;
    public TableColumn tbc_direccion;
    public TableColumn tbc_saldo;
    public TextField txf_nombre;
    public TextField txf_cedula;
    public TextField txf_direccion;
    public TextField txf_telefono;
    public TextField txf_correo;
    public TextField txf_contrasenia;
    public Button btn_volver;
    public Button btn_eliminarUsuario;
    public Button btn_actualizarUsuario;
    public Button btn_agregarUsuario;
    public Button btn_verCuentas;

    public void onVolver(ActionEvent actionEvent) {
        
    }

    public void onEliminarUsuario(ActionEvent actionEvent) {
        
    }

    public void onActualizarUsuario(ActionEvent actionEvent) {
        
    }

    public void onAgregarUsuario(ActionEvent actionEvent) {
        
    }

    public void onVerCuentas(ActionEvent actionEvent) {
        
    }
}